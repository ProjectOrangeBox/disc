...
class Disc
{
    ...
    protected static int $rootLength = 0;
    protected static bool $autoGenerateDirectories = true;

    /**
     * @param array<string, mixed> $configDisc
     */
    public static function configure(array $configDisc): void
    {
    ...
    public static function stripRootPath(string $files, bool $remove = true): string
    {
     ...
     */
    public static function stripRootPaths(array $files, bool $remove = true): array
    {
    ...
    public static function require(string $requiredPath): mixed
    {
        return require self::resolve($requiredPath, false, self::FILE);
    }

    public static function requireOnce(string $requiredPath): mixed
    {
        return require_once self::resolve($requiredPath, false, self::FILE);
    }

    public static function include(string $requiredPath): mixed
    {
        return include self::resolve($requiredPath, false, self::FILE);
    }

    public static function includeOnce(string $requiredPath): mixed
    {
    ...
    /**
     *
     * @param array<array-key, mixed> $files ie. $_FILES
     ...
     * @return array<array-key, mixed> array of moved files and other details
    ...
    {
        if ($timestamp === null) {
            // no timestamp is not a time to format; 0 is the epoch, which is
            // what every caller of this already treats a missing stat as
            return 0;
        }

        return $dateFormat === null ? $timestamp : date($dateFormat, $timestamp);
    }
    ...
    public static function cleanFilename(string $filename): string
    {
        ...
        do {
            // preg_replace() returns null if a pattern fails to compile
            $filename = preg_replace($nonDisplayables, '', (string) $filename, -1, $count) ?? $filename;
        } while ($count);
    ...
    {
        // opendir() returns false for a directory it cannot read, and
        // readdir()/closedir() cannot take that
        $dir = \opendir($source);
